...
sidecar. The fixed `kichad-production-10.0.4-v7` profile requires explicit stackup intent, ERC, DRC,
sourcing, and fabrication checks, plus Gerber, drill, IPC-D-356 electrical-test, placement, and BOM
outputs; STEP, BREP, GLB, STL, XAO, PDF, IPC-2581C XML, ODB++, front/back assembly SVG/DXF, GenCAD,
VRML, and typed board statistics are optional declarations. A native Eeschema netlist can also be
declared and is matched against the exact compiled KDS component and ref/pin connectivity sets. Each
auxiliary artifact has
