...
The native `fabricate` call plans and exports the fixed `kichad-production-10.0.4-v7` release
...
front/back SVG and DXF assembly drawings, GenCAD, VRML, BREP, GLB, STL, XAO, and typed JSON board
statistics; each is generated by KiCad 10 with an explicit deterministic profile and accepted only
by its dedicated bounded structural validator. A native KiCad s-expression netlist is also matched
exactly against the compiled KDS component and ref/pin connectivity sets. IPC-2581 is fixed to
