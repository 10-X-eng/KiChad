    ...
    JSON facets = JSON::array(
            {
                    ...
                    capability( "symbols.authoring", "symbols", "partial", { "symbol" },
                                "AI-native project symbol creation with metadata, custom fields, inclusion flags, numbered/common units, De Morgan body styles, all five native vector primitives with complete stroke/fill styles and RGBA color, and fully typed pins lowered to current KiCad 10 native libraries.",
                                { "Symbol text/text-box graphics, field layout, power/derived semantics, pin alternates, and publish workflows are not represented yet." } ),
