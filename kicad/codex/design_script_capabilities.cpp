    ...
    JSON facets = JSON::array(
            {
                    ...
                    capability( "pcb.vias", "pcb", "partial", { "board via" },
                                "Stable net-connected via position, diameter, drill, layer span, type, lock, explicit front/back tenting, covering and plugging, drill filling/capping, and front/back counterbore or countersink through the official KiCad 10 padstack IPC message.",
                                { "Full per-layer via copper shapes, custom primitives, secondary/tertiary drilling, layer-removal policy, and teardrops remain absent." } ),
                    ...
                    capability( "footprints.authoring", "footprints", "partial", { "footprint", "pad", "model" },
                                "KDS-owned current-format footprint definitions with mandatory fields, metadata, attributes, jumper/net-tie groups, standard/custom/chamfered pads, per-layer padstacks, all six native graphic primitives, rich text, rectangular/polygon text boxes, and project-local 3D assignments.",
                                { "Local properties and rules, zones, groups, variants, component classes, embedded assets, and complete package-data-to-KLC geometry synthesis remain absent." } ),
                    capability( "footprints.padstacks", "footprints", "partial", { "footprint pad" },
                                "SMD, connect, plated and non-plated through-hole pads with standard, chamfered, and semantic custom shapes; deterministic custom primitives; drills and offsets; mask/paste/clearance/thermal overrides; front/inner/back and sparse named-inner padstacks; tenting; post-machining; layer-removal policy; properties; and pin metadata.",
                                { "Per-layer custom-shape primitives, secondary/tertiary drilling, and footprint-level zone/group membership remain absent. Plugging, filling, capping, and covering are KiCad via rather than footprint-pad file semantics and remain gaps under pcb.vias." } ),
