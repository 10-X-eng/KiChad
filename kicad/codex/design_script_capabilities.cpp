    ...
    JSON facets = JSON::array(
            {
                    ...
                    capability( "schematic.directive_labels", "schematic", "partial",
                                { "directive" },
                                "Net-targeted directive labels with stable identity, flag geometry, and explicit field typography.",
                                { "Schematic rule areas and directives attached to their borders are not represented yet." } ),
                    capability( "schematic.text_graphics", "schematic", "unrepresented", {},
                                "Free text, text boxes, shapes, curves, images, tables, and table cells.",
                                { "These native schematic drawing object types have no KDS forms." } ),
                    ...
                    capability( "symbols.authoring", "symbols", "unrepresented", {},
                                "Symbol creation/editing, units, body styles, graphics, fields, pins, and pin alternates.",
                                { "KDS consumes symbol libraries but cannot author them." } ),
                    capability( "symbols.library_management", "symbols", "partial", { "library" },
                                "Project table generation and dependency validation.",
                                { "Create, rename, delete, migrate, rescue, and publish library content are absent." } ),
                    ...
                    capability( "pcb.vias", "pcb", "partial", { "board via" },
                                "Stable net-connected via position, diameter, drill, layer span, type, and lock.",
                                { "Full per-layer padstacks, custom shapes, tenting, plugging, filling, capping, and post-machining are absent." } ),
                    ...
                    capability( "footprints.authoring", "footprints", "unrepresented", {},
                                "Footprint definitions, pads, fields, graphics, text, zones, groups, attributes, and rule overrides.",
                                { "KDS consumes footprint libraries but cannot author them." } ),
                    capability( "footprints.padstacks", "footprints", "unrepresented", {},
                                "Pad types, drill/post-machining, custom per-layer shapes, mask/paste, thermal, and pin metadata.",
                                { "No footprint pad/padstack authoring form exists." } ),
                    capability( "footprints.models", "footprints", "unrepresented", { "library model" },
                                "3D model assignment, offset, rotation, scale, visibility, and embedded model data.",
                                { "Model roots can be declared, but footprint model assignments cannot be authored." } ),
                    capability( "footprints.library_management", "footprints", "partial", { "library" },
                                "Project footprint-table generation and dependency validation.",
                                { "Create, rename, delete, migrate, and publish footprint libraries are absent." } ),
                    ...
                    capability( "manufacturing.additional_exports", "manufacturing", "unrepresented", {},
                                "Assembly drawings, DXF/SVG layers, GenCAD, VRML, IDF, component netlists, reports, and custom BOM schemas.",
                                { "No KDS output forms or validators exist for these exports." } ),
                    ...
                    capability( "interchange.exporters", "interchange", "partial", { "output" },
                                "Production Gerber/drill/IPC/ODB/BOM/placement/STEP/PDF exports.",
                                { "Non-production netlist, graphics, mechanical, legacy, and report exporters remain absent." } ),
