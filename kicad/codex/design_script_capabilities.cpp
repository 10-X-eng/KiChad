    ...
    JSON facets = JSON::array(
            {
                    ...
                    capability( "pcb.vias", "pcb", "partial", { "board via" },
                                "Stable net-connected through/blind/buried/micro vias with either one circular diameter or a complete front/inner/back or arbitrary per-layer copper padstack; circle/rect/oval/trapezoid/roundrect/chamfer and semantic custom-primitive geometry; offsets; complete unconnected-layer ring policy; top/bottom backdrilling; tenting; covering; plugging; filling; capping; post-machining; and lock state through the official KiCad 10 padstack IPC message.",
                                { "Per-layer forced-flash policy and teardrops remain absent." } ),
                    ...
                    capability( "footprints.padstacks", "footprints", "partial", { "footprint pad" },
                                "SMD, connect, plated and non-plated through-hole pads with standard, chamfered, and semantic custom shapes; deterministic pad-wide and per-copper-layer custom primitives; primary drilling plus top/bottom backdrilling; offsets; mask/paste/clearance/thermal overrides; front/inner/back and sparse named-inner padstacks; tenting; post-machining; layer-removal policy; properties; and pin metadata.",
