...
JSON buildFabricationPlan( const JSON& aIr, const std::string& aFileStem )
{
    ...
    if( !aIr.at( "schematic" ).at( "nets" ).empty()
        && automaticNetCount + wiredNetCount + hierarchicalNetCount == 0 )
    {
        issues.push_back(
                { { "type", "label_only_schematic" },
                  ...
                  { "recovery",
                    "Omit presentation for automatic direct wiring and hierarchy synthesis, or "
                    "declare wired/hierarchical explicitly, then review every rendered page." } } );
    }
